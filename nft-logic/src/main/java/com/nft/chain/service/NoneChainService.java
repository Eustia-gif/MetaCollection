...
public class NoneChainService implements ChainAbstractService {
	...
	public String chainTransfer(String id) {
		...
		String limitGet = redisTemplate.opsForValue().get(Constant.LIMIT + Constant.CHAIN_TRANSFER_ARTWORK + id);
		if (StrUtil.isNotBlank(limitGet)) {
			return syncTransactionHash(id);
		}

		redisTemplate.opsForValue().set(Constant.LIMIT + Constant.CHAIN_TRANSFER_ARTWORK + id, "1", 60, TimeUnit.SECONDS);
		ThreadPoolUtils.getSyncChainPool().schedule(() -> {
			redissonClient.getTopic(Constant.CHAIN_SYNC_TRANSACTION_HASH).publish(toHoldCollection.getId());
		}, 35, TimeUnit.SECONDS);
		return "上链确认中";
	}
	...
	public String mintArtwork(String id) {
		...
		String limitGet = redisTemplate.opsForValue().get(Constant.LIMIT + Constant.CHAIN_MINT_ARTWORK + id);
		if (StrUtil.isNotBlank(limitGet)) {
			return syncUniqueId(id);
		}
		redisTemplate.opsForValue().set(Constant.LIMIT + Constant.CHAIN_MINT_ARTWORK + id, "1", 60, TimeUnit.SECONDS);
		ThreadPoolUtils.getSyncChainPool().schedule(() -> {
			redissonClient.getTopic(Constant.CHAIN_SYNC_UNIQUE_ID).publish(id);
		}, 35, TimeUnit.SECONDS);
		return "上链确认中";
	}
	...
	public String createBlockChainAddr(String id) {
		...
		String limitGet = redisTemplate.opsForValue().get(Constant.LIMIT + Constant.CREATE_BLOCKCHAIN_ADDRESS + id);
		if (StrUtil.isNotBlank(limitGet)) {
			return "处理中";
		}
		redisTemplate.opsForValue().set(Constant.LIMIT + Constant.CREATE_BLOCKCHAIN_ADDRESS + id, "1", 60, TimeUnit.SECONDS);
	...
	public String chainAddArtwork(String id) {
		...
		String limitGet = redisTemplate.opsForValue().get(Constant.LIMIT + Constant.CHAIN_CREATE_ARTWORK + id);
		if (StrUtil.isNotBlank(limitGet)) {
			return syncArtworkHash(id);
		}

		redisTemplate.opsForValue().set(Constant.LIMIT + Constant.CHAIN_CREATE_ARTWORK + id, "1", 60, TimeUnit.SECONDS);
		ThreadPoolUtils.getSyncChainPool().schedule(() -> {
			redissonClient.getTopic(Constant.CHAIN_SYNC_ARTWORK_HASH).publish(id);
		}, 35, TimeUnit.SECONDS);
		return "上链确认中";
	}
