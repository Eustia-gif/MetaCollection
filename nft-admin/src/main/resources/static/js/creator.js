...
var appVM = new Vue({
	...
	methods : {
		...
		handleUploaded : function(event){
			// event.xhr 是 XMLHttpRequest 对象，你可以从这个对象中获取服务器的响应
			let response = JSON.parse(event.xhr.responseText);
			// 从响应中获取图片的URL
			let imageUrl = response.data;
			// 将图片的URL赋值给 selectedRecord.avatar
			this.selectedRecord.avatar = imageUrl;

			console.log(imageUrl);
			console.log(response);
		},
		factoryFunction : function(file){
